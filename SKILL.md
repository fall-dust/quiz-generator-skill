...
| `js/editor.js` | **静态模板** | 全字段编辑模块（题目文本 + 选项内容 + 正确答案；运行时覆盖 + 编辑UI + 导出新版JS/编辑清单JSON） |
...
系统内置完整的题目编辑功能，可修改**题目文本、选项内容和正确答案**。从第一性原理出发：浏览器无法写文件 → 两条互补持久化路径：

```
第1层 运行时覆盖 ── localStorage → 启动时覆盖 q.question / q.options / q.answer，对 app.js 透明
第2层 编辑 UI ──── 侧边栏开关 + MutationObserver 注入编辑控件 + 弹窗编辑器
...
| 字段 | 选择题 | 大题（填空/简答/计算） |
|------|:---:|:---:|
| 📝 题目文本 | ✅ textarea | ✅ textarea |
| 📋 选项内容 | ✅ 每个选项独立 input | — |
| ✅ 正确答案 | ✅ radio/checkbox 选择 | ✅ textarea |

### 编辑流程

```
侧边栏「🔧 题库编辑」→ 开启「✏️ 编辑题目/选项/答案」
  → 每道题目出现 ✏️ 按钮（已修改的显示 🔶 标记）
  → 点击弹出全字段编辑器：
      选择题：题目 textarea + A/B/C/D 选项 input × 4 + 正确答案选择器
      大题：题目 textarea + 答案 textarea
  → 保存 → localStorage 存储覆盖（增量合并，只存修改的字段）
  → 自动重判用户原回答（App.reevaluate）
  → 导出：
      📥 导出新版 questions.js → 浏览器下载完整文件，手动替换
...
**完整编辑闭环：**
```
浏览器编辑 → 导出编辑清单 JSON → python apply_edits.py --apply → 源文件已更新
```
...
| `.ae-edit-opts` / `.ae-opt` / `.ae-sel` | 正确答案选项列表 / 选项 / 选中态 |
| `.ae-edit-opts-list` / `.ae-opt-row` | 选项文本编辑列表 / 单行（label + input） |
| `.ae-opt-label` / `.ae-opt-text-input` | 选项标签（A/B/C/D）/ 选项文本输入框 |
| `.ae-edit-field` / `.ae-lbl` / `.ae-textarea` | 编辑字段容器 / 字段标签 / 文本输入区 |
