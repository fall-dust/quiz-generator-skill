...
**核心特点：** 自适应解析引擎 → 精准提取数据 → **原生 JS 渲染架构** → 五题型独立（选择题/判断题/填空题/简答题/计算题），空题型自动隐藏。
...
| 文件 | 生成方式 | 说明 |
|------|---------|------|
| `index.html` | **静态模板** | 纯 HTML 容器结构；默认引入 Quill/Marked/KaTeX/DOMPurify/highlight.js (CDN) |
| `css/style.css` | **静态模板** | CSS 变量驱动的完整样式系统，含 Quill 主题适配 + 图片缩放 + 修改详情面板 |
| `js/questions.js` | **自动生成** | 选择题数据 `var QUESTIONS = [...]` |
| `js/bigquestions.js` | **自动生成** | 大题数据 `var BIG_QUESTIONS = [...]` |
| `js/app.js` | **静态模板** | 原生 JS 应用层（状态管理 + 富文本渲染管线 + 事件 + 答案重判） |
| `js/editor.js` | **静态模板** | Quill 富文本编辑器（题目/答案 WYSIWYG + 图片拖拽缩放 + 运行时覆盖 + 修改详情面板 + 导出） |
| `js/lib/download-libs.py` | **静态模板** | Python 下载脚本：将第三方库下载到 `js/lib/`，确保离线可用 |
| `apply_edits.py` | **静态模板** | Python 脚本：命令行直接修改 `questions.js`（题目/选项/答案全字段），支持从浏览器编辑清单 JSON 批量应用 |

**依赖策略：** 默认通过 CDN 引入 Quill/Marked/KaTeX/DOMPurify/highlight.js 实现富文本编辑和渲染。需要离线运行时，运行 `python js/lib/download-libs.py` 下载到本地 `js/lib/` 目录；本地缺失时自动回退 CDN。
...
├── editor.js             ├── js/editor.js      # 全字段编辑模块（题目+选项+答案）
...
应用从源数据**自动探测**实际存在的题型，空题型自动隐藏。标准支持n种题型：

| 题型 | 模式 | 数据源 | 进度追踪 | 视图 |
|------|------|--------|---------|------|
| ✏️ 选择题 | `mcq` | `QUESTIONS[_origType!=判断]` | `answers{}` + `results{}` | 章节网格 → 选项点击 |
| ⚖️ 判断题 | `tf` | `QUESTIONS[_origType==判断]` | `answers{}` + `results{}` | 章节网格 → 选项点击 |
...
系统启动时通过 `typeHasQuestions(mode)` 检测每种题型是否有题目，**只显示有题目的题型**。侧边栏按钮和仪表盘卡片自动过滤空题型（`display:none`）。
...
通过 `mcqFiltered(mode)` 基于 `_origType` 精确过滤：
- `mcq` 模式：`_origType !== '判断'`（单选 + 多选）
- `tf` 模式：`_origType === '判断'`（仅判断题）

每种题型：
- 独立按章节浏览：`mode=tf + chapter=ch2` → 只看第2章判断题
- 章节角标反映当前题型进度（选择题/判断题显示"已答/总数"，大题显示"已记住/总数"）
- 侧边栏统计随题型切换
- 仪表盘显示各题型独立的进度条
...
| **数据源** | `getQs()` — 根据 mode 返回对应数据集（mcq/tf 通过 `mcqFiltered` 分离） | IIFE 闭包 |
| **题型探测** | `mcqFiltered(mode)` / `typeHasQuestions(mode)` — 选择题/判断题分离 + 空题型检测 | IIFE 闭包 |
...
| **主渲染** | `render()` — 组装各区域 HTML → innerHTML；侧边栏/仪表盘仅显示 `availTypes` | IIFE 闭包 |
...
| **版本迁移** | `migrateData()` — 架构变更时自动清理不兼容的旧数据（`net_hist`/`net_atest`） | IIFE 闭包 |
...
```js
1. QUESTIONS 中的 choice/判断/multiple 类型：
   - answer: "C"    →  ["C"]        （字符串→数组）
   - answer: "ABD"  →  ["A","B","D"] （多选题答案拆分）
   - type: "choice" → type: "single"（单选题统一）
   - type: "multiple" → 保留 "multiple"（不强制转换！）
   - type: "判断" → type: "single"，_origType = "判断"
   - 判断类型若无 √/× 选项则自动补全
...
系统在 `DOMContentLoaded` 时自动执行 `migrateData()`：对比 `APP_VERSION` 与 localStorage 中的版本号，架构变更时自动清除不兼容的旧数据（自测历史 `net_hist`、`net_atest`），用户答题进度不受影响。
...
系统默认通过 CDN 引入 5 个库实现富文本编辑和 Markdown 渲染。运行 `python js/lib/download-libs.py` 可下载到本地以实现离线运行。无需任何构建工具或 npm。
...
## 富文本渲染管线（Quill + Marked + KaTeX）

系统默认启用富文本渲染。`index.html` 通过 CDN 引入 Quill/Marked/KaTeX/DOMPurify/highlight.js 五个库，`app.js` 中的渲染函数和 `editor.js` 中的 Quill 编辑器均依赖它们。

### 渲染管线（app.js）

```
原始文本 → 保护数学公式(占位符) → Marked 解析 → KaTeX 还原 → highlight.js 高亮 → DOMPurify 过滤
```

- **`renderMarkdown(text)`**: 自动识别 HTML vs Markdown — 如果文本含 HTML 标签且不含 markdown 语法标记，走 `DOMPurify.sanitize()` 直通；否则走完整管线（Marked → KaTeX → hljs → DOMPurify）
- **`richText(text)`**: 包裹 `<div class="rich-content">` 容器，所有题目的 `q.question` 和答案展示均通过此函数
- **数学公式**: `$...$` 行内公式, `$$...$$` 块级公式 → KaTeX 渲染
- **代码块**: 三个反引号围栏 → highlight.js 自动高亮
- **降级容忍**: 任一 CDN 加载失败自动回退纯文本（`escapeHtml`），页面不崩溃

### CDN 依赖 (index.html `<head>`)

| 库 | 版本 | 用途 |
|---|---|---|
| Quill | 1.3.7 | WYSIWYG 富文本编辑器 (snow 主题) |
| Marked | 15.0 | Markdown → HTML 解析 |
| KaTeX | 0.16.21 | LaTeX 数学公式渲染 |
| highlight.js | 11.11 | 代码块语法高亮 |
| DOMPurify | 3.2 | HTML 安全过滤 (XSS 防护) |

### Quill 编辑器容器 (index.html)

在 `</body>` 前的 `#modalContainer` 之后，放置一个全局复用的 Quill 编辑弹窗：

```html
<div id="quillEditorContainer" class="quill-editor-overlay" style="display:none">
  <div class="quill-editor-dialog">
    <div class="quill-editor-header"><h3>✏️ 编辑题目</h3><button>✕</button></div>
    <div class="quill-editor-body">
      <div id="quillEditorQuestion" class="quill-editor-instance"></div>
      <div id="quillEditorAnswer" class="quill-editor-instance"></div>
      <div id="quillOptionsField"><!-- MCQ 选项编辑 --></div>
      <div id="quillMcqAnswerField"><!-- MCQ 答案选择 --></div>
    </div>
    <div class="quill-editor-footer">按钮区</div>
  </div>
</div>
```

注意：选择题仅使用 `#quillEditorQuestion`（题目为 Quill，答案用 radio/checkbox）；大题使用 `#quillEditorQuestion` + `#quillEditorAnswer`（题目和答案均为 Quill）。

---

## 全字段编辑模块（Quill 富文本编辑器 + 三层架构）

系统内置完整的 Quill 富文本编辑功能。从第一性原理出发：浏览器无法写文件 → 两条互补持久化路径：

```
第1层 运行时覆盖 ── localStorage → 启动时覆盖 q.question / q.options / q.answer，支持 HTML (Quill 产物)
第2层 编辑 UI ──── 侧边栏开关 + MutationObserver 注入编辑控件 + Quill WYSIWYG 弹窗
第3层 持久化 ──── 路径A: 浏览器下载完整新版 questions.js
                 路径B: 导出编辑清单 JSON → python apply_edits.py 直接修改源文件
```
...
| 字段 | 选择题 | 大题（填空/简答/计算） |
|------|:---:|:---:|
| 📝 题目文本 | ✅ Quill 编辑器 | ✅ Quill 编辑器 |
| 📋 选项内容 | ✅ 每个选项独立 `<input>` | — |
| ✅ 正确答案 | ✅ radio/checkbox 选择 | ✅ Quill 编辑器 |

### 编辑流程

```
侧边栏「🔧 题库编辑」→ 开启「✏️ 富文本编辑」
  → 每道题目出现 ✏️ 按钮（已修改的显示 🔶 标记）
  → 点击弹出 Quill 编辑弹窗：
      选择题：题目 Quill + A/B/C/D 选项 <input> × 4 + 正确答案选择器
      大题：题目 Quill + 答案 Quill（两个独立的 Quill 实例）
  → 保存 → localStorage 存储 HTML（Quill 产物）
  → 自动重判用户原回答（App.reevaluate）
  → 导出：
      📥 导出新版 questions.js → 浏览器下载完整文件
      📋 导出编辑清单 JSON → python apply_edits.py 直接修改源文件
```

### Quill 编辑器生命周期管理（关键设计）

Quill 实例的创建/销毁必须严格管理，**否则会出现工具栏重复、键盘事件失效等问题**：

1. **`destroyAllQuills()`** — 在 `openEditor()` 首行调用：
   - 禁用旧 Quill 实例 (`enable(false)`)
   - 解绑键盘事件 (`keyboard.detach()`)
   - 删除容器 `__quill` 引用
   - 调用 `_destroyQuillDOM()` 清理每个容器的 DOM
   - **安全网扫描**：搜索整个 overlay 内的 `.ql-toolbar`，全部移除
2. **`_destroyQuillDOM(el)`** — 清理单个容器：
   - **仅移除紧邻容器的前置兄弟 `.ql-toolbar`**（snow 主题把 toolbar 插在容器正前面，**不是容器内部**）。注意必须只移除紧邻的，不能用 while 遍历全部前置兄弟，否则大题场景下 answer 容器会误删 question 的工具栏
   - 删除 `el.__quill` 引用
   - 清除 `ql-container`/`ql-snow` class，清空 innerHTML
3. **`_openTimeout`** — `setTimeout` 句柄，防止快速连续打开/关闭导致竞态
4. **`initQuill(containerId, content)`** — 创建 Quill 实例：
   - 先调用 `_destroyQuillDOM(el)` 确保容器干净
   - 创建 `new Quill(el, { theme: 'snow', modules: { toolbar: toolbarCfg } })`
   - 设置初始内容（HTML 走 `dangerouslyPasteHTML`，纯文本走 `setText`）
   - 安装图片粘贴处理器（Ctrl+V blob → FileReader → base64 data URL）
   - 调用 `setupImageResize(quill)` 安装图片拖拽缩放
...
**插入方式：**
- **工具栏按钮**：点击图片按钮 → 系统文件选择器（`<input type="file" accept="image/*">`）→ FileReader 转 base64 → `insertEmbed` → `insertEmbed('image', url)`
- **Ctrl+V 粘贴**：粘贴板检测到图片 blob → `FileReader` 转 base64 data URL → `insertEmbed`

**拖拽缩放（JS 驱动）：**
CSS `resize: both` 在 contenteditable 内的 `<img>` 上**无效**。系统使用 `setupImageResize(quill)` 实现 JS 驱动的四角拖拽缩放：

| 机制 | 实现 |
|------|------|
| **手柄检测** | 6 个方向 — 4 角 (`nw`/`ne`/`sw`/`se`) + 2 边 (`e`/`s`) |
| **动态光标** | 靠近角落时自动切换 `nwse-resize` / `nesw-resize` / `ew-resize` / `ns-resize` |
| **拖拽缩放** | `mousedown` 角 → `preventDefault` → `document.mousemove` 实时改 `width`/`height` → `mouseup` 结束 |
| **等比例** | 始终保持原始宽高比，最小尺寸 30×20px |
| **视觉反馈** | hover → 虚线框；靠近角落 → `.qimg-resizable` 实线外框 |

**CSS 图片样式：**
```css
.quill-editor-instance .ql-editor img {
  max-width:100%; height:auto; display:inline-block;
  cursor:default; border:2px dashed transparent;
  outline:2px solid transparent; transition:border-color,outline-color; padding:2px; vertical-align:top
}
.quill-editor-instance .ql-editor img:hover { border-color: var(--p) }
.quill-editor-instance .ql-editor img.qimg-resizable {
  outline-color: var(--p); outline-offset: 2px; border-color: var(--p)
}
```

### 修改详情面板

侧边栏修改计数旁提供"查看详情"链接，打开独立弹窗展示所有修改：

- **标题行**：题目 ID（等宽紫色） + 章节 + 题型 + ✏️ 编辑 / ↩ 还原按钮
- **字段 diff**：红色左边框（旧值） → 绿色左边框（新值），HTML 已剥离为纯文本预览（100 字符截断）
- **底部操作**：🗑 清除全部 / 📋 导出清单 JSON
- **交互**：点击遮罩/✕/ESC 关闭、还原单题即时刷新列表、编辑按钮打开 Quill 编辑器并关闭详情面板
- **z-index**：比编辑器低 2 级，不会遮挡编辑弹窗

### 覆盖层数据结构

`editor.js` 维护两个核心数据结构：

```js
Overrides._data     // { qId: { question: '<p>HTML</p>', options: [...], answer: '...' } }
                    // 只存储被修改的字段（增量合并）

Overrides._originals // { qId: { question: '原文', options: [...], answer: '...' } }
                    // 首次修改时快照原始值，用于还原和 diff 展示
```
...
### apply_edits.py — 命令行直接修改源文件

从浏览器导出编辑清单 JSON 后，用此脚本直接修改 `questions.js`（题目/选项/答案全字段）：

```bash
# 从浏览器编辑清单批量应用（推荐工作流）
python apply_edits.py --apply edit_manifest.json

# 或命令行直接修改单题
python apply_edits.py --id q10 --question "修正后的题目文本"
python apply_edits.py --id q10 --answer D
python apply_edits.py --id q10 --option A "修正后的选项A文本"

# 批量修改选项
python apply_edits.py --id q10 --options A:文本1 B:文本2 C:文本3 D:文本4

# 预览不写入 / 列出全部题目
python apply_edits.py --id q10 --answer D --dry-run
python apply_edits.py --list

# 跳过备份
python apply_edits.py --apply manifest.json --no-backup
```

**完整编辑闭环：**
```
浏览器 Quill 编辑 → 导出编辑清单 JSON → python apply_edits.py --apply → 源文件已更新
```

自动备份原文件为 `.bak`，支持 100+ 选择题和大题的解析→修改→重建闭环。
...
| 选择器 | 功能 |
|--------|------|
| `.quill-editor-overlay` | 编辑弹窗遮罩层 (z-index: `--z-lg + 10`) |
| `.quill-editor-dialog` | 编辑弹窗对话框 (max-width: 820px) |
| `.quill-editor-header` / `.quill-editor-close` | 标题栏 / 关闭按钮 |
| `.quill-editor-body` | 可滚动内容区 |
| `.quill-editor-field` / `.quill-editor-label` | 编辑字段容器 / 字段标签 |
| `.quill-editor-instance` | **Quill 编辑器容器**（`min-height: 150px`，focus-within 蓝色边框） |
| `.quill-editor-instance .ql-toolbar.ql-snow` | Quill 工具栏（border-radius 与容器对齐） |
| `.quill-editor-instance .ql-container.ql-snow` | Quill 编辑区 |
| `.quill-editor-instance .ql-editor` | Quill 内容区（min-height: 120px, line-height: 1.7） |
| `.quill-editor-footer` | 按钮区（取消/还原/保存） |
...
| `.ae-toggle-btn` / `.ae-active` | 编辑模式开关按钮 / 开启态（黄色边框） |
| `.ae-toggle-state` | 开关状态标识（关/开） |
| `.ae-export-btn` | 导出按钮（JS 文件 / JSON 清单） |
| `.ae-count` | 修改计数 + 查看详情 / 清除全部 链接 |
| `.ae-hint` | 侧边栏提示文字 |
...
| 选择器 | 功能 |
|--------|------|
| `.ae-edit-btn` / `-hd` / `-bq` | 编辑按钮 / 卡片头部 / 大题列表 |
| `.ae-overridden-badge` | 已修正标记 |

**修改详情面板：**

| 选择器 | 功能 |
|--------|------|
| `.changes-detail-overlay` | 详情面板遮罩 (z-index: `--z-lg + 12`) |
| `.changes-detail-dialog` | 详情面板对话框 (max-width: 760px) |
| `.changes-detail-header` / `.changes-detail-close` | 标题栏 / 关闭按钮 |
| `.changes-detail-body` | 可滚动内容区 |
| `.changes-detail-footer` | 底部按钮区 |
| `.changes-list` | 修改条目列表 (flex, gap: 14px) |
| `.changes-item` / `.changes-item-hd` | 单条修改 / 标题行 (ID + 章节 + 题型 + 操作按钮) |
| `.changes-item-id` | 题目 ID (紫色 monospace) |
| `.changes-item-ch` / `.changes-item-type` | 章节标签 / 题型标签 |
| `.changes-btn-edit` / `.changes-btn-restore` | ✏️ 编辑 / ↩ 还原 按钮 |
| `.changes-fields` | 变更字段区 |
| `.changes-field` / `.changes-field-label` | 单个字段 / 字段名 |
| `.changes-diff` | 新旧值对比行 |
| `.changes-old` | 旧值 (红色左边框, max-width: 280px, 截断省略) |
| `.changes-new` | 新值 (绿色左边框, max-width: 280px, 截断省略) |
| `.changes-arrow` | → 箭头 |
...
| 选择器 | 功能 |
|--------|------|
| `.quill-options-list` / `.quill-opt-row` | 选项编辑列表 / 单行 |
| `.quill-opt-label` | 选项标签（A/B/C/D，紫色加粗） |
| `.quill-opt-text-input` | 选项文本输入框 |
| `.quill-mcq-answer-select` / `.quill-mcq-ans-opt` | 正确答案选择区 / 选项 |
| `.quill-mcq-ans-opt.selected` | 已选中（绿色边框 + 加粗） |

**富文本显示（app.js 渲染的题目卡片）：**

| 选择器 | 功能 |
|--------|------|
| `.rich-content` | 富文本内容容器 (font-size: `--q-size`, line-height: `--q-lh`) |
| `.rich-content img` | 已渲染图片 (max-width: 100%) |

---

## 编码规范

1. **安全**：用户输入用 `escapeHtml()` 转义；Quill 产物通过 `DOMPurify.sanitize()` 过滤后插入
2. **CSS 变量简写**：`--p` 主色、`--t` 文本、`--cd` 卡片、`--t2` 次要文本；`--q-size: 1.2rem` / `--q-lh: 1.85` 题目字号/行高
3. **输出路径**：默认在题库同目录创建 `{输出目录}/`
4. **交叉验证**：生成后提取 `index.html` 中所有 `class="..."` 值，逐个检查是否在 `style.css` 中有 `.xxx` 定义
5. **`window.App` 命名空间**：全部操作函数挂在 `App` 对象上，不污染全局；`App.render` / `App.reevaluate` / `App.toast` 暴露供 editor.js 调用
6. **数据文件只含数据**：`questions.js` 和 `bigquestions.js` 仅定义 `QUESTIONS` / `BIG_QUESTIONS` 变量，不含逻辑
7. **错题/收藏重做答案隔离**：使用 `_wrAns/_wrRes` 临时对象，退出时同步
8. **答案覆盖透明化**：`editor.js` 启动时应用 localStorage 覆盖到内存对象（支持 HTML），app.js 无需感知
9. **Quill 生命周期管理**：每次 `openEditor()` 首行调用 `destroyAllQuills()`，`_destroyQuillDOM()` 仅移除紧邻前置 toolbar 兄弟节点，`initQuill()` 包含 `setupImageResize()`
10. **降级容忍**：任一 CDN 库加载失败 → `app.js` 自动回退 `escapeHtml()` 纯文本，不白屏
11. **富文本渲染**：`richText(q.question)` 替代 `escapeHtml(q.question)`；`renderMarkdown()` 自动识别 HTML/Markdown
12. **编辑器数据**：`editor.js` 的 `Overrides._data` 增量存储修改字段（可能含 HTML），`Overrides._originals` 首次修改时快照原始值
